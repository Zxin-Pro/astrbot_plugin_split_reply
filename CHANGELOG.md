...
- **QQ 官方机器人原生流式支持**：新增 `native_stream` 配置（`auto` / `on` / `off`，默认 `auto`）。
  - 开启后，`streaming_split` 模式下每条分段不再走普通发送，而是各自开一条**平台原生流式消息**——对 QQ 官方机器人（C2C 私聊）来说就是官方流式协议（`state=1` 分片刷新 → `state=10` 收尾），客户端呈现**逐字蹦出**的效果
  - `auto`：检测到 `qq_official` / `qq_official_webhook` 平台自动启用；其他平台（如 aiocqhttp）走普通分段发送
  - 实现细节：pump 协程持续消费上游增量，每凑满一行立刻建一条队列（内容 + 结束哨兵），主流程按序对每段调用一次原 `send_streaming`，因此每段都是独立的一条流式消息，且**不必等整轮 LLM 结束**才收尾
  - 兼容：非文本组件单独成段、`break`（工具调用分隔）信号透传给平台实现、分段间延迟照常生效、`max_length` 截断照常生效
- 新增 14 项原生流式分段单测（总计 69 项全部通过）
